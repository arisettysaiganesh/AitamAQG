...
# Set the path to wkhtmltopdf executable (adjust if needed)
pdfkit_config = pdfkit.configuration(wkhtmltopdf=r'C:\Program Files\wkhtmltopdf\bin\wkhtmltopdf.exe')


def add_password_to_pdf(pdf_file, password):
    """
    Encrypts a PDF file with the provided password.

    Args:
        pdf_file (str): Path to the existing PDF file.
    ...
    try:
        ...
        print(f"Password to be used: {password}")  # Added logging

        # Encrypt the PDF with the provided password
        writer.encrypt(user_pwd=password, owner_pwd=None, use_128bit=True)
        
        print(f"PDF encrypted with password: {password}")  # Add print statement


        # Write the encrypted PDF to a new file (consider renaming)
        with open(f"{pdf_file}_encrypted.pdf", 'wb') as output_pdf:
            writer.write(output_pdf)

        print("PDF successfully encrypted with password!")
